...
exports.update = function (collection, type, callback) {
        ...
        db.collection(collection + 's', function (err, col) {
          ...
          col.count(function (err, count) {
            ...
            col.find({}, {limit: 1}).toArray(function (err, docs) {
              ...
              var newEntity = null;
              if (type === 'tsv') {
                newEntity = new Entity(collection, 'TSV_Project', collection + '.tsv',
                  collection, 'daily', 'prod', properties, 'completed', count);
              } else {
                newEntity = new Entity(collection, 'CSV_Project', collection + '.csv',
                  collection, 'daily', 'prod', properties, 'completed', count);
              }
